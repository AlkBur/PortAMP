...
import "github.com/AlkBur/PortAMP/al"
...
// Velocity returns the source's looping.
func (s Source) Looping() int32 {
	return s.Geti(paramSourceLooping)
}

// SetLooping sets the source's looping.
func (s Source) SetLooping(v bool) {
	if v {
		s.Seti(paramSourceLooping, al.AL_TRUE)
	}else{
		s.Seti(paramSourceLooping, al.AL_FALSE)
	}
}

// SetLooping sets the source's looping.
func (s Source) SetBuffer(buf Buffer) {
	s.Seti(paramSourceBuffer, int32(buf))
}
